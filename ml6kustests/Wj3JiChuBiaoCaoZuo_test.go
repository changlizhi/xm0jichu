...
func TestXinZeng(t *testing.T){
  ...
  jiChuKu := []map[string]interface{}{}
  yongHuKu := map[string]interface{}{}
  jiChuKu=append(jiChuKu,yongHuKu)
  ...
  jueSeZiYuanKu[ml2changliangs.ZiDuans]=jszyzd
    ...
    // log.Println("TestXinZeng,i,v---",i,v)
    //组装好所有的字段送到dao层进行入库
    canShu:=ml3moxings.CanShu{}
    canShu.ShuJu=[]map[string]interface{}{}
    
    canShuShuJu1 := jiChuKu[ml2changliangs.Sz0]//为什么要for循环后这样再组装成CanShu一个一个的传入？为了用for循环模拟真实使用时的情况，因为真实使用时不可能一次性传入这么多数据，肯定是一条一条提交上来的。特别是新增的时候，除非要用导入，这就是另外的问题了。
    canShu.ShuJu=append(canShu.ShuJu,canShuShuJu1)
